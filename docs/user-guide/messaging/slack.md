...
slack:
  ...
  # When true, thread replies in multi-bot threads require an explicit mention.
  # Single-bot threads still allow natural follow-up without re-mentioning.
  multi_bot_mentions_only: false
...
:::info
Slack does not have a `free_response_channels` equivalent. By default, the adapter requires `@mention` to start a conversation in channels, then allows follow-up replies inside the same thread without repeated mentions. If you enable `slack.multi_bot_mentions_only: true`, that follow-up behavior changes only for threads where two or more bots have already participated: single-bot threads stay natural, but multi-bot threads require explicit mentions on each follow-up. In DMs, the bot always responds without needing a mention.
:::
